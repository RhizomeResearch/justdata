    ...
    *,
    ...
    if mask is None:
        ...
        if mask.dtype not in (tf.uint8, tf.uint16, tf.int16, tf.int32, tf.int64):
            raise TypeError("semantic mask must have an integer categorical dtype")
        ...
        allowed = tf.concat(
            [tf.cast(class_ids, tf.int64), [tf.cast(ignore_value, tf.int64)]], 0
        )
        tf.debugging.assert_equal(
            tf.reduce_all(tf.reduce_any(labels[..., None] == allowed, axis=-1)),
            True,
